...
import { coerceToRational } from '#src/rational.js'
import { coerceToFloatTest, coerceToFloatThrowsErrorTest } from '#test/float/coerceToFloat.js'
...
describe('coerceToRational', () => {
	test(coerceToFloatThrowsErrorTest.message, () => {
		coerceToFloatThrowsErrorTest.data.forEach(({ input: { a, b }, error }) => {
			assert.throws(() => { coerceToRational(a, b) }, error)
		})
	})
