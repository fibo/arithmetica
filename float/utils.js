export const base10FractionToFloat = (bigInt, denominatorBase10Exponent) => {
	...
	if (bigInt < 0n) {
		...
		if (bigIntStringLength - 1n === denominatorBase10Exponent) {
			...
			return removeRightPaddedZerosFromNonRepeatingDecimal("-0." + "0".repeat(Number(denominatorBase10Exponent - bigIntStringLength + 1n)) + bigIntString.slice(1));
		} else {
			return removeRightPaddedZerosFromNonRepeatingDecimal(
				"-" +
					bigIntString.slice(1, Number(bigIntStringLength - denominatorBase10Exponent)) +
					"." +
					bigIntString.slice(Number(bigIntStringLength - denominatorBase10Exponent))
			);
		}
	} else {
		if (bigIntStringLength === denominatorBase10Exponent) {
			...
			return removeRightPaddedZerosFromNonRepeatingDecimal("0." + "0".repeat(Number(denominatorBase10Exponent - bigIntStringLength)) + bigIntString);
		} else {
			return removeRightPaddedZerosFromNonRepeatingDecimal(
				bigIntString.slice(0, Number(bigIntStringLength - denominatorBase10Exponent)) +
					"." +
					bigIntString.slice(Number(bigIntStringLength - denominatorBase10Exponent))
			);
		}
...
export const removeRightPaddedZerosFromNonRepeatingDecimal = (bigIntString) => {
	...
	if (i === bigIntStringLength - 1) return bigIntString;
	if (bigIntString.charAt(i) === ".") return bigIntString.slice(0, i);
	else return bigIntString.slice(0, i + 1);
};
